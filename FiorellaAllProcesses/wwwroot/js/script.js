$(document).ready(function () {
    $(document).on("click", "#more", async function () {
        var proCount = $(".plus").children().length;
        let dbproCount = $("#proCount").val();
        let data = await fetch('https://localhost:44361/Product/LoadProduct?skip=' + proCount);
        let fetchData = await data.text();
        $(".plus").append(fetchData);
        var proCount = $(".plus").children().length;
        if (proCount == dbproCount) {
            $("#more").remove();
        }
    })
